...
# =============== Paths ===============
cell_path = '/home/skshastry/Project/rosmap_brain_blood/data/cell_type_embd.json'
tcr_path  = '/home/skshastry/Project/rosmap_brain_blood/data/tcr_embeddings.json'
...
X_tcr_all   = np.stack(merged_data['tcr_emb'].apply(np.asarray).to_list()).astype(np.float32)   # (N, d_tcr)
X_age_all   = merged_data['Age'].to_numpy(np.float32)[:, None]                                   # (N,1)

g = merged_data['Gender'].to_numpy(np.float32)[:, None]
g1d = g.reshape(-1)                         # (N,)
g_idx = g1d.astype(np.int64) 

X_sex_all = np.eye(2, dtype=np.float32)[g_idx]   # shape (N, 2), one-hot
X_cell_emb=np.stack(merged_data['cell_emb'].apply(np.asarray).to_list()).astype(np.float32)
...
X_all = np.concatenate([X_cell_flat, X_tcr_all, X_age_all,X_sex_all], axis=1).astype(np.float32)
D_cell=X_cell_flat.shape[1]
D_tcr=X_tcr_all.shape[1]
AGE_COL=D_cell+D_tcr
SEX_START  = AGE_COL + 1               
SEX_END    = SEX_START + 2
...
class PatientClassifier(L.LightningModule):
    def __init__(self, d_model: int, n_cls: int, dropout_rate: float, nlayers: int = 1,
                 ...
                 weight_decay: float = 0.0):           # L2 reg -prevents overfittings (acts on model parameters (w))
        super().__init__()
    ...
    def objective(trial):
        ...
        for tr_idx, va_idx in inner_cv.split(X_outer_train, y_outer_train):
            ...
            mlf_logger = MLFlowLogger(
                experiment_name='NN_cv_optuna_ADIS',
